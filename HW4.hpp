...
private:
	...
	void move_to_L1(block_t block)
	{
		...
		if (line.valid)
		{
			...
			{
				cerr << "L2 inclusion violation - NRU ML1" << endl;
				exit(1);
			}
		}
		line.valid = true;
	...
	{
		_stats.l2_block_fills++;
		CacheLine &line = _l2_cache[L2_IDX(block)].find_NRU(_l2_mru[L2_IDX(block)]);
		if (line.valid)
		{
			if (line.hits == 0)
				_stats.l2_evicts_at_0_hit++;
	...
	bool l1_lookup(block_t block)
	{
		...
		if (line.valid && line.block == block)
		{
			...
			{
				cerr << "L2 inclusion violation - NRU" << endl;
				exit(1);
			}
			return true;
		}
